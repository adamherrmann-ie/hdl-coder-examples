...
simOut = sim('matrix_for_each_wrapper','FastRestart','off','SrcWorkspace','current','ReturnWorkspaceOutputs','on', 'StopTime', sprintf('%d',stopTime));
model_stop_time = toc;
disp("Finished simulink model " + model_stop_time + " seconds");

% Extract valid data
valid_out_para = simOut.output_valid.Data;
data_out_para = simOut.output_data_0.Data(valid_out_para,:);